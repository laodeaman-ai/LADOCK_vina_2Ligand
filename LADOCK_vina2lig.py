...
# Simulasi khusus main_pdbqt
main_output_file = f"{main_pdbqt.replace('.pdbqt', '_output.pdbqt')}"
main_command = f'./vina_1.2.5_linux_x86_64 --receptor {receptor_pdbqt} --ligand {main_pdbqt} --config config.txt --out {main_output_file}'
os.system(main_command)
print(f"Simulasi selesai untuk {main_pdbqt}. Output: {main_output_file}")
...
# Simulasi Ligan Ganda
for pdbqt_file in pdbqt_files:
    output_file = f"{main_pdbqt.replace('.pdbqt', '')}_{pdbqt_file.replace('.pdbqt', '')}_output.pdbqt"    
    command = f'./vina_1.2.5_linux_x86_64 --receptor {receptor_pdbqt} --ligand {main_pdbqt} {pdbqt_file} --config config.txt --out {output_file}'
    os.system(command)
    print(f"Simulasi selesai untuk kombinasi {main_pdbqt} dengan {pdbqt_file}. Output: {output_file}")
...
# Menghapus direktori "model1" beserta isinya jika sudah ada
if os.path.exists(model1_dir):
    shutil.rmtree(model1_dir)
